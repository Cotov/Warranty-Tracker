package bg.softuni.warranty_tracker.constant;

public class ErrorMessages {

    public static final String USERNAME_ALREADY_EXISTS = "User with this username already exists";
    public static final String EMAIL_ALREADY_EXISTS = "User with this email already exists";
    public static final String INVALID_LOGIN_CREDENTIALS = "Username or password incorrect";
    public static final String AUDIT_ENTRIES_FETCH_FAILED = "Failed to fetch audit entries with status code: %s";

}
